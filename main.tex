...
called the interval monoid, which formalizes the notion of indexing the
subarrays being accessed over the course of the prefix sum
computation.~\cite{Chong2014}

For convenience, we quote the relevant definition:

\begin{definition}{((4.3) of \cite{Chong2014})}
...
operations which are array slices; they are produced by Julia code like
...
for the subarray \code{y[i:k]}.  The identity element $\mathrm 1$ formalizes
...
\code{UnitRange} of the form \code{i:j}.
...
\item 	A function that computes the prefix sum in serial is correct for $n$
	...
	which are 0-based.} Furthermore, the correct answer is
	$\left((1, 1), (1, 2), \dots, (1, n) \right)$, as the $k$th partial sum
	involves summing the subarray $y[1:k]$.
	
\item	A function that computes the prefix sum in parallel is correct if it is
	free of data races and its equivalent serialization has the correctness
	property above.
...
Julia types, the definition \eqref{def:intervalmonoid} translates directly into
the following code:

\begin{minted}[mathescape,texcl]{julia}
#S is the interval monoid of Definition \ref{def:intervalmonoid}
abstract Tee #$\top$
abstract Id  #$\mathbf 1_I$
typealias S Union(UnitRange, Type{Tee}, Type{Id}) #$\mathbb S_I$

#$\oplus_I$
+(I::UnitRange, J::UnitRange) = 
    I.stop+1==J.start ? (I.start:J.stop) : Tee
+(::Type{Id}, ::Type{Id}) = Id
+(I::S, ::Type{Id}) = I
+(::Type{Id}, I::S) = I
+(::Type{Tee}, ::Type{Tee}) = Tee
+(::Type{Id}, ::Type{Tee}) = Tee
+(::Type{Tee}, ::Type{Id}) = Tee
+(I::S, ::Type{Tee}) = Tee
+(::Type{Tee}, I::S) = Tee
\end{minted}

Some additional methods for \code{+} are needed to resolve method ambiguities
in Julia, which currently resolves method dispatch only on the types of the
input arguments and not on the output type.
...
Attempting to verify an incorrect kernel results in at least one \code{Tee}
being produced in the program state, thus precipitating type conversion errors
of the form
...
\end{minted}
%
which arise from the inability of noncontinuous ranges to be expressed as
\code{UnitRange}s.
