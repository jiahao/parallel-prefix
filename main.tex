...
MPI provides the \code{MPI\_scan} primitive, and in MPI-2, also the \code{MPI\_Exscan} primitive for exclusive scan.
...
What is prefix reduction~\cite{Iverson1962,Ladner1980}? Making use of associativity (or approximate associativity side node about floating point and how it doesn't really matter for most applications)  to regroup operations to provide different execution strategies.
...
- stream compaction ( M. Harris, S. Sengupta, and J. Owens. Parallel prefix sum (scan) with
CUDA. In H. Nguyen, editor, GPU Gems 3. Addison-Wesley, 2007.)

- parallel sort (Blelloch 1989; N. Satish, M. Harris, and M. Garland. Designing efficient sorting
algorithms for manycore GPUs. In IPDPS, pages 1–10, 2009.)

- polynomial interpolation ( O. Egecio ˘ glu, E. Gallopoulos, and C. Koc¸. A parallel method ˘
for fast and practical high-order Newton interpolation. BIT Numerical
Mathematics, 30:268–288, 1990.)

- list operatioins (Hillis and Steele; Gorlatch 1999)

- solving linear systems of equations involving block tridiagonal matrices (Seal 2012)

- solving recurrences

- minimal coverings of black-and-white images (Moitra, 1991)

- string matching problems (Chi and Hui, 2005)

- random number generation (Lu, Hou and Chen, 1996)

- simulating finite state machines (Ladner and Fisher 1980)
...
Monoidal structure has been used to construct a formal algebra of scan algorithms (Hinze)

\section{Related work}

What do other languages do?

Other languages also provide scan primitives like APL (Iverson 1962), ZPL, MPI.

Other approaches wish they have genericity, like the Thrust C++ library whihc uses crazy C++ functors.

Does Haskell have something using monoids?

Haskell Accelerate for GPU programming. Is it generic? Yes because it works at the code generation level, taking in expressions and emitting longer expressions that compute the prefix sum. In Haskell land these things are criticised for being not statically typed...? (Chakravarty et al, Accelerating Haskell array codes with multicore GPUs) 

Repa doesn't have parallel scan.
...
Other variants: e.g. Snir, Koc, doubly pipelined (Sanders and Träff), harmonically scheduled (Wang, Nicolau and Siu, 1996), segmented scan (Sengupta 2007).

Generalization to nonassocative binary operations (Chen and Hou, 1992)
